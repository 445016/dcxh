	...
	{	
		...
		if(empty($index_sort_method)){
			$sql_pingoods = "select {$fields} from "
        	           ...
        	           where  {$where}   and g.id=gc.goods_id and type !='pin' and type !='integral'  order by  g.istop DESC, g.settoptime DESC,g.id desc  limit {$offset},{$perpage} ";
		}else{
			$sql_pingoods = "select {$fields} from "
                        .C('DB_PREFIX')."lionfish_comshop_goods as g,".C('DB_PREFIX')."lionfish_comshop_good_common as gc   
        	           where  {$where}   and g.id=gc.goods_id and type !='pin' and type !='integral'  order by  g.index_sort desc,g.id desc  limit {$offset},{$perpage} ";
		}
