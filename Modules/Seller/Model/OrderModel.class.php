...
class OrderModel{
					...
					for($s = 1; $s <= $page; $s++) {
						...
						if( !empty($list) )
						{
							foreach($list as $val)
							{
								...
								$head_commiss_order = M('lionfish_community_head_commiss_order')->where( array('order_id' => $val['order_id'],'order_goods_id' => $val['order_goods_id']) )->select();
	...
	**/
	...
	{
		$order_info = M('lionfish_comshop_order')->field('order_id,order_num_alias,total,member_id')->where( array('order_num_alias' => $order_sn) )->find();
		if(!empty($order_info)){
