    ...
    public function index()
	{
		...
		if (empty($_GPC['reports_index']) || $_GPC['reports_index']=='0'){
			...
			foreach($day_info as $val1) {
				...
				$zongxiadan += $val1['total']+$val1['shipping_fare']-$val1['voucher_credit']-$val1['fullreduction_money'] - $val1['score_for_money'];
			...
			//取消订单
			$quxiao_info = M()->query("select from_unixtime( pay_time, '%Y-%m-%d' ) as date, count( * ) as count,sum(total) as total,sum(shipping_fare) as shipping_fare,sum(voucher_credit) as voucher_credit,sum(fullreduction_money) as fullreduction_money,sum(score_for_money) as score_for_money from ".C('DB_PREFIX')."lionfish_comshop_order where pay_time > ".$thisweek['egt']." and pay_time < ".$thisweek['lt']." ".$condition." and (order_status_id = 5) group by date order by date asc");
			...
			foreach($quxiao_info as $val4) {
					...
					$quxiao += $val4['total']+$val4['shipping_fare']-$val4['voucher_credit']-$val4['fullreduction_money']+ $val4['score_for_money'];
			}
		...
		if($_GPC['reports_index'] == 1){
			...
			//有订单的所有日期
			$day_info2 = M()->query("select from_unixtime( date_added, '%Y-%m-%d' ) as date from ".C('DB_PREFIX')."lionfish_comshop_order where pay_time > ".$lastweek['egt']." and pay_time < ".$lastweek['lt']." ".$condition." group by date order by date asc" );
