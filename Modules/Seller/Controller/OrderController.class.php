	...
	public function oprefund_goods_do()
	{
		...
		**/
		...
		if ( IS_POST ) {
			...
			//部分退款计算运费
			if($refund_quantity < $order_goods_info['quantity']){
	...
	**/
	...
	public function detail()
	{
        ...
        switch ($item['delivery']){
            case 'pickup':$item['delivery_'] = '自提';break;
			...
			if( !empty($head_commission_order_info) )
			{
				foreach( $head_commission_order_info as  &$vv)
				{
					...
					if($vv['type'] == 'orderbuy' && $vv['add_shipping_fare'] > 0){
						$refund_shipping_sql = "SELECT refund_shipping_fare "
						...
						$vv['money'] = $vv['money']-$refund_shipping_fare;
					}else{
	...
	public function opreceive()
	{
		...
		if( $item['delivery'] == 'localtown_delivery' )
        {
            ...
            D('Home/LocaltownDelivery')->distribution_arrived_order( $item['order_id']  );
        }
		...
		if (IS_POST) {
			...
			if($cansub == 1)
			{
				...
				if($res['code'] == 1)
				{
					...
					//退款配送费问题
					  D('Seller/Commonorder')->ins_order_goods_refund($refund_info['order_id'], $refund_info['order_goods_id'],$pay_total_money,$order_refund['real_refund_quantity'], $order_refund['real_refund_quantity'],$order_refund['ref_money']-$order_refund['ref_shipping_fare'],1,$order_refund['ref_shipping_fare']);
	...
	public function thirth_delivery_order(){
		...
		//商品信息
		$sql = "select og.goods_id,og.name as goods_name,og.quantity,g.weight as goods_weight  from "
		...
		foreach($goods_list as $k=>$v){
			...
			$goods_weight = $goods_weight + $v['quantity']*$v['weight'];
