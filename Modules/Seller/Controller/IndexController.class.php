...
class IndexController extends CommonController {
	...
	public function analys ()
	{
		...
		$list = M('lionfish_comshop_order')->where("1 ".$score_for_money_where )->select();
		$sum_score = 0;
		foreach( $list as $key => $val )
		{
			$od_info = M('lionfish_comshop_member_integral_flow')->field('score')->where( array('order_id' => $val['order_id'] ) )->find();
